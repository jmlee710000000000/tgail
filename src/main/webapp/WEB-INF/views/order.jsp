								...
								<c:forEach items="${orderList}" var="ol">
									<tr>
										...
										<fmt:formatNumber value="${ol.totalPrice}" pattern="#,### 원" />
											<input type="hidden" class="individual_shipPrice_input"
											value="${ol.shipPrice}"> <input type="hidden"
											class="individual_shipCount_input" value="${ol.shipCount}">
											<input type="hidden" class="individual_shipId_input"
											value="${ol.shipId}">
										</td>
									</tr>
								</c:forEach>
			...
			$(".goods_table_price_td").each(
					function(index, element) {
						...
						totalPrice += parseInt($(element).find(
								".individual_shipPrice_input").val());
						// 총 갯수
